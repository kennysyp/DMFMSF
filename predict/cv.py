...
side_info_dim = params.pca_dim
tf.set_random_seed(params.tf_random_seed)


def validate(data_set, model, sess, roc_params=False):
    XL_batch, U_batch, XR_batch, V_batch, Y_batch = dl.coor_to_sample(data_set, use_sise_info=True)
    y_pred, y_score, accuracy, loss = sess.run(
        [model.prediction, model.score, model.accuracy, model.loss],
        feed_dict={model.XL_input: XL_batch, model.U_input: U_batch, model.XR_input: XR_batch, model.V_input: V_batch,
...
def cross_validation(dl):
    model = DMF_model.DMF(use_side_info=True, feature_dim=side_info_dim)
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epoch_num):
            ...
            for iter, indices in enumerate(range(0, dl.train_size, batch_size)):
                ...
                XL_batch, U_batch, XR_batch, V_batch, Y_batch = dl.coor_to_sample(X_coor_batch, use_sise_info=True)
                y_pred, y_score, loss, reg_loss, _ = sess.run(
                    ...
                    feed_dict={model.XL_input: XL_batch, model.XR_input: XR_batch,
                               model.U_input: U_batch, model.V_input: V_batch, model.Y_input: Y_batch})
...
dl = data_input.DataLoader(use_side_info=True)
# 保存矩阵每一列的预测结果
ys = []
xs = []
#
